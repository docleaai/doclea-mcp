/**
 * Tests for LibSqlVectorStore
 * Tests vector storage, search, and filtering with libSQL's native vector support
 */
...
describe("LibSqlVectorStore", () => {
  ...
  describe("initialization", () => {
    test("creates tables on initialize", async () => {
      const newStore = new LibSqlVectorStore({ dbPath: ":memory:" });
  ...
  describe("constructor options", () => {
    test("accepts string path", async () => {
      const newStore = new LibSqlVectorStore(":memory:");
      await newStore.initialize();
      const info = await newStore.getCollectionInfo();
      expect(info.vectorsCount).toBe(0);
      newStore.close();
    ...
    test("uses custom table name", async () => {
      const newStore = new LibSqlVectorStore({
        dbPath: ":memory:",
