...
describe("ChangeDetector", () => {
	...
	describe("detectChanges", () => {
		it("should detect added files (no stored hash)", async () => {
			const files = [
				{ path: "new-file.ts", content: "export const foo = 1;" },
			];
	...
	describe("updateHashes", () => {
		it("should persist hashes for added files", async () => {
			...
			const row = db.query("SELECT hash FROM file_hashes WHERE path = ?").get("new.ts") as any;
		...
		it("should update hashes for modified files", async () => {
			...
			const row = db.query("SELECT hash FROM file_hashes WHERE path = ?").get("test.ts") as any;
		...
		it("should remove hashes for deleted files", async () => {
			...
			const row = db.query("SELECT hash FROM file_hashes WHERE path = ?").get("delete-me.ts");
			expect(row).toBeNull();
		});
		...
		it("should handle empty changes array", async () => {
			...
			// Should not throw
			const count = db.query("SELECT COUNT(*) as count FROM file_hashes").get() as any;
	...
	describe("Edge Cases", () => {
		...
		it("should handle files with special characters in path", async () => {
			...
			const row = db.query("SELECT path FROM file_hashes WHERE path = ?").get(files[0].path) as any;
