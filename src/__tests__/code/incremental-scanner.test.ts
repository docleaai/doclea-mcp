...
import type { VectorStore, VectorPayload, VectorSearchResult } from "../../vectors/interface";

// Mock VectorStore for testing
class MockVectorStore implements VectorStore {
	private vectors = new Map<string, { vector: number[]; payload: VectorPayload }>();
	...
	async getCollectionInfo(): Promise<{ vectorsCount: number; pointsCount: number }> {
	...
	getVector(id: string): { vector: number[]; payload: VectorPayload } | undefined {
...
class MockEmbeddingClient implements EmbeddingClient {
	async embed(text: string): Promise<number[]> {
		...
		return Array(384).fill(0).map((_, i) => Math.sin(i + text.length) * 0.1);
	}
...
describe("IncrementalScanner", () => {
	...
	return "hello world";
			...
			// First scan
			await scanner.scanIncremental([{ path: "/test/example.ts", content: initialContent }]);

			// Modify and rescan
			const result = await scanner.scanIncremental([{ path: "/test/example.ts", content: modifiedContent }]);
...
export function goodbye(): string {
	return "goodbye";
		...
		it("should ignore unchanged files", async () => {
			const files = [{ path: "/test/example.ts", content: "export const x = 1;" }];
