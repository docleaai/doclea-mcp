...
describe("getUnsummarized", () => {
	...
	it("should return empty when no nodes need summaries", async () => {
		...
		db.run(
			`UPDATE code_nodes SET metadata = ?`,
			[JSON.stringify({ summaryGeneratedBy: "ai", summaryConfidence: 0.95, needsAiSummary: 0 })],
		);
...
describe("batchUpdateSummaries", () => {
	...
	beforeEach(() => {
		...
		db.run(
			...
			[
				...
				"src/utils.ts",
				"Function doSomething",
				JSON.stringify({ summaryGeneratedBy: "signature", summaryConfidence: 0.5, needsAiSummary: 1 }),
				now,
				now,
			],
		);
		...
		const result = await batchUpdateSummaries(
			{
				summaries: [
					...
					},
					{
						...
						summary: "Handles incoming HTTP requests and routes them appropriately",
					},
				],
			},
			db,
	...
	it("should use transactions for efficiency", async () => {
		...
		const count = db.query("SELECT COUNT(*) as count FROM code_nodes WHERE summary LIKE 'Summary for function%'").get() as any;
