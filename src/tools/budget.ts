 ...
 */
const _BudgetCategorySchema = z.enum(["system", "context", "user", "response"]);
 ...
 * Allocate token budget across categories
 */
export async function allocateBudget(input: AllocateBudgetInput): Promise<{
 ...
 * Get available model context windows
 */
export async function getModelWindows(_input: GetModelWindowsInput): Promise<{
  models: Array<{ name: string; contextWindow: number }>;
}> {
  const models = Object.entries(MODEL_CONTEXT_WINDOWS).map(
    ([name, window]) => ({
      name,
 ...
 */
export async function getBudgetPresets(input: GetBudgetPresetsInput): Promise<{
  presets: Record<
    string,
    {
      ratios: Record<BudgetCategory, number>;
