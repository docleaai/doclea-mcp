...
import type { CachedEmbeddingClient } from "../embeddings/provider";
...
async function buildRAGContext(
	...
	for (const result of results) {
		const content = formatMemory(result);
...
function formatImplementations(iface: any, implementations: any[]): string {
	...
	parts.push(`Interface \`${iface.name}\` has ${implementations.length} implementations:`);
