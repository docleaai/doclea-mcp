...
export class TokenBudgetManager {
	...
	private validateConfig(): void {
		...
		if (Math.abs(sum - 1.0) > 0.001) {
			throw new Error(
				`Budget ratios must sum to 1.0, got ${sum.toFixed(3)}`,
			);
		}
	...
	allocate(): BudgetAllocation {
		...
		const categories: BudgetCategory[] = ["system", "context", "user", "response"];
		...
		if (remaining > 0) {
			...
			const priority: BudgetCategory[] = ["context", "user", "response", "system"];
